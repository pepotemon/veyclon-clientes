...
export default function VentasNuevasScreen({ route }: Props) {
  ...
  // Filtra a ventas (préstamos creados) y mapea a MovimientoItem
  const items: MovimientoItem[] = useMemo(() => {
    const ventas = raw
      .filter((r) => canonicalTipo(r?.tipo) === 'prestamo')
      .map((r) => {
        const createdMs =
        ...
        const title =
          ...
          (r?.clienteId ? `Cliente ${String(r.clienteId).slice(0, 6)}…` : 'Venta');
    ...
    ventas.sort((a, b) => (b._createdMs || 0) - (a._createdMs || 0));

    // Quitamos campo interno
    return ventas.map(({ _createdMs, ...rest }) => rest);
  }, [raw]);
  ...
  return (
    <InformeDiario
      ...
      total={total}
      loading={loading}
      emptyText="No hay ventas para esta fecha."
      onRefresh={reload}
    />
  );
}

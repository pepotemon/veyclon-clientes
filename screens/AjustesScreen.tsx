...
export default function AjustesScreen({ navigation }: Props) {
  ...
  const data: Item[] = [
    { key: 'prefs',   title: 'Preferencias del usuario', subtitle: 'Personaliza la app', icon: 'options' },
    { key: 'enrutar', title: 'Enrutar Clientes', subtitle: 'Seleccionar', icon: 'map' },
    { key: 'cerrar',  title: 'Cerrar Día', subtitle: 'Resumen y corte de caja', icon: 'calendar' },
    { key: 'caja',    title: 'Definir Caja', subtitle: 'Opción solo para revisadores', icon: 'cash' },
  ];

  const goWithAdmin = async (cb: (admin: string) => void) => {
    try {
  ...
  const onPress = (item: Item) => {
    switch (item.key) {
      ...
      case 'cerrar':
        ...
        break;
      // en AjustesScreen.tsx, dentro de onPress:
      case 'caja':
...
const styles = StyleSheet.create({
  ...
  iconCircle: {
    width: 30, height: 30, borderRadius: 15, alignItems: 'center', justifyContent: 'center', borderWidth: 1,
  },

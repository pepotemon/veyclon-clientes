...
const dispRef = (clienteId) => db.collection('clientesDisponibles').doc(clienteId);
const clienteRef = (clienteId) => db.collection('clientes').doc(clienteId);
/** Asegura documento en el índice y “siembra” datos básicos para listar rápido. */
async function ensureIndiceCliente(clienteId) {
    const [cSnap, dSnap] = await Promise.all([
        clienteRef(clienteId).get(),
        dispRef(clienteId).get(),
    ]);
    const base = { id: clienteId };
    if (cSnap.exists) {
        const c = cSnap.data() || {};
        ...
        if (c.tenantId !== undefined)
            base.tenantId = c.tenantId ?? null; // ✔ tenant
    }
    if (!dSnap.exists) {
...
async function normalizeDisponibilidad(clienteId) {
 ...
 */
export const onPrestamoCreated = onDocumentCreated('clientes/{clienteId}/prestamos/{prestamoId}', async (event) => {
    ...
    try {
        ...
        if (!cajaSnap.exists) {
            ...
            const monto = Number(p.valorNeto ?? p.monto ?? p.montoTotal ?? p.totalPrestamo ?? 0) || 0;
...
export const onPrestamoDeleted = onDocumentDeleted('clientes/{clienteId}/prestamos/{prestamoId}', async (event) => {
    const clienteId = String(event.params.clienteId || '');
    if (!clienteId)
        return;
    await ensureIndiceCliente(clienteId);
...
'clientes/{clienteId}/prestamos/{prestamoId}/abonos/{abonoId}', async (event) => {
    const { clienteId, prestamoId, abonoId } = event.params;
    if (!clienteId || !prestamoId || !abonoId)
        return;
    ...
    await db.runTransaction(async (tx) => {
        ...
        const restanteActual = Number(p.restante ?? p.totalPrestamo ?? p.montoTotal ?? 0) || 0;
        const valorCuota = Number(p.valorCuota || 0);
        const monto = Number(abono.monto || 0);
        const cuotasTotales = Number(p.cuotasTotales || p.cuotas || 0) ||
            (valorCuota > 0
        ...
        const nuevasCuotasPagadas = cuotasTotales > 0
    ...
    try {
        ...
        if (!p2.exists)
            return;
        const prestamo = p2.data();
        await prestamoRef.set({ ultimaReconciliacion: FieldValue.serverTimestamp() }, { merge: true });
